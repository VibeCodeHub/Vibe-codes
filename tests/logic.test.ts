...
import { createEmptyBoard, collides, tryRotate, tryMove, clearLines, scoringForLines, initGame, stepGravity, applyHardDrop } from '../src/game/logic';
import { GRID_W } from '../src/game/constants';
import type { ActivePiece, Board } from '../src/game/types';
import { SevenBagRNG } from '../src/game/rng';

function makePiece(kind: ActivePiece['kind'], x: number, y: number, rotation: ActivePiece['rotation'] = 0): ActivePiece {
  ...
  it('clears 1..4 lines and awards scores', () => {
    for (let lines = 1; lines <= 4; lines++) {
      ...
      for (let y = 0; y < lines; y++) {
        for (let x = 0; x < GRID_W; x++) board[19 - y][x] = { colorId: 0 };
      }
