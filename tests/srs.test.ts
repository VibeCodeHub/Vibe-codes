...
import { getKicks, nextRotation, SHAPES } from '../src/game/srs';
import { collides } from '../src/game/logic';
import { createEmptyBoard } from '../src/game/logic';
import type { PieceKind, Rotation } from '../src/game/types';

// Canonical SRS kick tables from Tetris Guideline
// Reference: https://tetris.wiki/Super_Rotation_System
const CANONICAL_I_KICKS: Record<`${Rotation}-${Rotation}`, Array<{x: number, y: number}>> = {
...
const CANONICAL_T_KICKS: Record<`${Rotation}-${Rotation}`, Array<{x: number, y: number}>> = {
...
function testKickTable(kind: PieceKind, from: Rotation, to: Rotation, canonical: Array<{x: number, y: number}>) {
  const ourKicks = getKicks(kind, from, to);
  expect(ourKicks).toEqual(canonical);
}

function testRotationNearWall(kind: PieceKind, x: number, y: number, from: Rotation, to: Rotation, dir: -1 | 1) {
...
describe('SRS Conformance Tests', () => {
  ...
  describe('Wall/corner rotation tests', () => {
    ...
    it('I piece cannot rotate in left corner (correct SRS behavior)', () => {
      ...
      const board = createEmptyBoard();
      const piece = { kind: 'I' as const, x: 0, y: 18, rotation: 0 as const };
