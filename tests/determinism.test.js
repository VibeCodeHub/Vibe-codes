...
function applyInputScript(state, rng, inputs) {
    let current = state;
    for (const input of inputs) {
        switch (input.action) {
            case 'move':
                current = applyMove(current, input.params[0]);
                break;
            case 'rotate':
                current = applyRotate(current, input.params[0]);
                break;
            case 'hardDrop':
                ...
                const spawn = { x: 4, y: 0, rotation: 0 };
