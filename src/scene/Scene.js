...
function PerformanceMonitor() {
    ...
    useFrame((state) => {
        ...
        window.__perf__.fps = fps;
        window.__perf__.quality = quality;
        // Check every 500ms for responsiveness
        if (now - lastCheck.current > 500) {
            lastCheck.current = now;
            // Skip if in cooldown
            if (now - lastChange.current < cooldownMs)
                return;
            ...
            if (recent1s.length < 30 || recent3s.length < 90)
...
function DebugUpdater() {
    ...
    useFrame(() => {
        if (window.__perf__) {
            window.__perf__.dpr = gl.getPixelRatio();
            window.__perf__.calls = gl.info.render.calls;
        }
    });
    return null;
}
export default function Scene(props) {
    ...
    return (_jsxs(Canvas, { camera: { fov: 35, position: cameraPos.toArray() }, dpr: dpr, gl: { antialias: true }, onCreated: ({ gl, scene, camera }) => {
        ...
        }, children: [_jsx(AdaptiveDpr, { pixelRatio: [1, 2] }), _jsx(Suspense, { fallback: null, children: _jsx(Environment, { preset: "city", environmentIntensity: enableTransmission ? 1.0 : 0.5 }) }), enableBloom && (_jsx(EffectComposer, { children: _jsx(Bloom, { mipmapBlur: true, intensity: 0.3, luminanceThreshold: 0.85, luminanceSmoothing: 0.1, radius: 0.4 }) })), _jsx(Performance, {}), _jsx(PerformanceMonitor, {}), _jsx(DebugUpdater, {}), props.children] }));
}
