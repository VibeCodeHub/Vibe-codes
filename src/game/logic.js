...
export function pieceCells(kind, rotation, origin) {
...
export function collides(board, kind, rotation, origin) {
...
export function scoringForLines(lines, level) {
    const base = LINE_CLEAR_SCORES[lines] ?? 0;
    return base * (level + 1);
}
export function kindToColorId(kind) {
    switch (kind) {
        case 'I': return 0;
        case 'J': return 1;
        case 'L': return 2;
...
export function spawnPosition(kind) {
...
export function initGame(seed) {
    ...
    const active = { kind: first, rotation: spawn.rotation, x: spawn.x, y: spawn.y };
    const state = {
        board,
        active,
        hold: null,
...
export function stepGravity(state, rng) {
    if (!state.active)
        return state;
    ...
    const newActive = { kind: nextKind, rotation: spawn.rotation, x: spawn.x, y: spawn.y };
